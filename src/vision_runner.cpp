...
void VisionRunner::run() {
  ...
  const auto object_detection_ms =
      duration_cast<milliseconds>(object_detection_time - start_time).count();
  ...
  for (const auto &obj : objects.object_list) {
    ...
    auto vision_pose = Messages::CreateVisionPose(
        builder, obj.id, obj.position.x, obj.position.y, obj.position.z,
        object_detection_ms);
    vision_pose_offsets.push_back(vision_pose);
    //	    }
  }
  auto poses_vector = builder.CreateVector(vision_pose_offsets);
  auto vision_pose_array =
      Messages::CreateVisionPoseArray(builder, poses_vector);
  ...
  zmq_manager_->get_publisher("BackZed").publish_prebuilt(
      "Objects", builder.GetBufferPointer(), builder.GetSize());
  const auto pipeline = high_resolution_clock::now();
  const auto pipeline_ms =
      duration_cast<milliseconds>(pipeline - start_time).count();
