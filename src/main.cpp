...
enum MODE { BUILD_ENGINE, PLAYBACK, REALTIME, TAG_DETECTION, INVALID };
...
MODE args_interpreter(int argc, char **argv) {
  ...
  if (std::string(argv[1]) == "-s" && argc >= 4) {
    ...
    detector.init_detector(img_width, img_height, tile_size, tag_family,
                           tag_dim, decimate);
    ...
    for (const auto &tag : detectedTags) {
      ...
      for (int i = 0; i < 4; ++i) {
        LOG_INFO("Corner ", i, ",: {", tag.corners[i].x, ", ", tag.corners[i].y,
                 "}");
        cv::line(
            image, cv::Point(tag.corners[i].x, tag.corners[i].y),
...
#else
    LOG_ERROR(
        "CUDA not available on this device, cannot perform tag detection!");
    return INVALID;
#endif
  }
  return REALTIME;
