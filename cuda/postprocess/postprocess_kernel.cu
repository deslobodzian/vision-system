...
static __global__ void nms_kernel(CudaBBoxInfo *bbox_infos, int num_boxes,
                                  float nms_threshold) {
...
static __global__ void postprocess_kernel(
    float *predictions, int num_anchors, int num_classes, float obj_threshold,
    float xOffset, float yOffset, float scalingFactor_x, float scalingFactor_y,
    int width, int height, CudaBBoxInfo *bbox_infos, int *valid_count) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  if (max_score > obj_threshold) {
    ...
    float x0 =
        clamp((x - 0.5f * w) * scalingFactor_x, 0.f, static_cast<float>(width));
    float y0 = clamp((y - 0.5f * h) * scalingFactor_y, 0.f,
                     static_cast<float>(height));
    float x1 =
        clamp((x + 0.5f * w) * scalingFactor_x, 0.f, static_cast<float>(width));
    float y1 = clamp((y + 0.5f * h) * scalingFactor_y, 0.f,
                     static_cast<float>(height));
...
void postprocess(const Tensor<float> &prediction, std::vector<BBoxInfo> &bboxs,
  ...
  float scaling_factor = std::min(static_cast<float>(input_w) / image_w,
  ...
  postprocess_kernel<<<grid, block>>>(
      prediction.data(), num_anchors, num_classes, obj_thres, x_offset,
      y_offset, scaling_factor_x, scaling_factor_y, image_w, image_h,
