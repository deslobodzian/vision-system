...
void zmq_objects_subscriber_thread(ZmqManager& zmqManager, const std::string& topic, std::shared_ptr<nt::NetworkTable> vision_table, std::atomic<bool>& running) {
	...
	while (running.load()) {
		try {
			...
			if (message) {
				...
				auto elapsed_time = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - last_message_time).count();
				if (elapsed_time > VISION_TIMEOUT_MS) {
					vision_table->GetEntry("num_objects").SetDouble(0);
					vision_table->GetEntry("vision_poses_raw").SetRaw(std::vector<uint8_t>());
				}
			}
		}
