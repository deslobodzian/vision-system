...
TEST(LoggerTest, LogLevels) {
  ...
  std::this_thread::sleep_for(std::chrono::milliseconds(100));  // Wait for async logging
...
TEST(LoggerTest, MultiThreadedLogging) {
  ...
  for (int i = 0; i < num_threads; ++i) {
    std::string debug_message = "Debug message from thread " + std::to_string(i);
    std::string info_message = "Info message from thread " + std::to_string(i);
    std::string error_message = "Error message from thread " + std::to_string(i);
    
    EXPECT_TRUE(log_output.find(debug_message) != std::string::npos) << "Thread " << i;
    EXPECT_TRUE(log_output.find(info_message) != std::string::npos) << "Thread " << i;
    EXPECT_TRUE(log_output.find(error_message) != std::string::npos) << "Thread " << i;
  }
