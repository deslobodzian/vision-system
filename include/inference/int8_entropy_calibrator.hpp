...
public:
    Int8EntropyCalibrator2(int batch_size, const std::string& calibration_data_path, 
    ...
    bool getBatch(void* bindings[], const char* names[], int nbBindings) noexcept override {
        ...
        cv::Mat img = cv::imread(image_paths_[current_image_index_], cv::IMREAD_UNCHANGED);
        if (img.empty()) {
    ...
    const void* readCalibrationCache(size_t& length) noexcept override {
        std::cout << "Searching for calibration cache: " << calibration_table_name_  << std::endl;
        calibration_cache_.clear();
        std::ifstream input(calibration_table_name_, std::ios::binary);
        input >> std::noskipws;
        if (read_cache_ && input.good()) {
            std::cout << "Reading calibration cache: " << calibration_table_name_  << std::endl;
    ...
    void writeCalibrationCache(const void* cache, size_t length) noexcept override {
        std::cout << "Writing calib cache: " << calibration_table_name_<< " Size: " << length << " bytes" << std::endl;
...
private:
    ...
    void loadCalibrationData(const std::string& calibDataDirPath) {
        ...
        if (!doesFileExist(calibDataDirPath)) {
            throw std::runtime_error("Error, directory does not exist: " + calibDataDirPath);
        }
        ...
        if (image_paths_.size() < static_cast<size_t>(batch_size_)) {
            throw std::runtime_error("Fewer calibration images than specified batch size!");
        }

        // Randomize the order of the calibration images
        std::shuffle(image_paths_.begin(), image_paths_.end(), std::default_random_engine(std::random_device{}()));
