...
public:
    ...
    ObjectDetector() {
        model_ = std::make_unique<Yolo<MatType>>("yolov8n.engine");
        detections_.reserve(100); // There should never be more that 100 object detected at once
    }
    ~ObjectDetector() {
    }

    void configure(const detection_config& config) {
        model_->configure(config);
    }

    void detect_objects(Camera& camera) {
