...
inline cv::Rect cvt_rect(const BBox& box) {
  return cv::Rect(round(box.x1), round(box.y1), round(box.x2 - box.x1),
                  round(box.y2 - box.y1));
}

inline cv::Rect sl_cvt_rect(const std::vector<sl::uint2>& box) {
  return cv::Rect(round(box[0].x), round(box[0].y), round(box[2].x - box[0].x),
                  round(box[2].y - box[0].y));
}
...
inline std::vector<sl::uint2> rect_to_sl(const cv::Rect& rect_in) {
  ...
  bbox_out[2] =
      sl::uint2(rect_in.x + rect_in.width, rect_in.y + rect_in.height);
