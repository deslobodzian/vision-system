...
template <typename T> inline T uniform_random(T center, T range) {
  return random(center - range, center + range);
}

template <typename T> inline T sample_triangle_distribution(T b) {
  return (sqrt(6.0) / 2.0) * (random(-b, b) + random(-b, b));
}

template <typename T> inline T zero_mean_gaussian(T x, T sigma) {
