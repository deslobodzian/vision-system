...
//    Task(TaskManager* manager, double period, std::string name);
    Task(std::shared_ptr<TaskManager> manager, float period, const std::string& name);
    virtual ~Task(); 
...
public:
    ...
    std::shared_ptr<T> create_task(Args&&... args) {
        static_assert(std::is_base_of<Task, T>::value, "T must be a derivative of Task");

        auto task = std::make_shared<T>(this->shared_from_this(), std::forward<Args>(args)...);
        add_task(task);
        return task;
    }
    void stop_tasks();
...
public:
    ...
    void init() override {}
    void run() override { if (function_) function_(); }
...
public:
    PeriodicMemberFunction(std::shared_ptr<TaskManager> taskManager, float period,
            ...
            : Task(taskManager, period, name),
              function_(std::bind(function, obj)) {}
