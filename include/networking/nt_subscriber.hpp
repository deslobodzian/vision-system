...
public:
    ...
    NTSubscriber(const nt::NetworkTableInstance &instance, const std::string &table, const subscribable &s) {
        auto nt_table = instance.GetTable(table);
        s_ = s;
        sub_ = nt_table->GetDoubleArrayTopic(s.get_topic()).Subscribe({});
    }

    void get() {
//        const std::lock_guard<std::mutex> lock(s.subscriber_mtx_);
